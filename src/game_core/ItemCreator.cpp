...
void ItemCreator::BuildAttributes() {
  ...
  for (const auto&[name, type] : pairs) {
	if (kAttributes.find("attributes." + name) == kAttributes.not_found()) continue;
	const auto kValue = kAttributes.get<std::int32_t>("attributes." + name);
	item_attributes.emplace_back(Attribute{type, kValue});
  }
