...
ItemBuilder::ItemBuilder(std::uint32_t template_id) {
  ...
  item_template_ = data.get_child("itemTemplates." + template_id_);
}

void ItemBuilder::MakeItem() {
  if (!BuildRawItem()) {
	item_.reset();
	return;
  }
  if (!BuildItemRequirements()) {
	item_.reset();
	return;
  }
  if (!BuildAttributes()) {
...
std::shared_ptr<Item> ItemBuilder::GetItem() const {
  return item_;
}
...
bool ItemBuilder::BuildItemRequirements() {
  try {
...
bool ItemBuilder::BuildOptionalData() {
  try {
	if (const auto it = item_template_.find("description"); it != item_template_.not_found()) {
	  //const auto desc = it->second.get<std::string>("description");
